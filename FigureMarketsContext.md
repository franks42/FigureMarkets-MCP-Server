...
- **The only mechanism to generate ROI** from available_unvested_amount is through **delegation to Provenance validators**
...
└── delegated_total_amount (delegation to validators - all buckets earn rewards)
    ├── delegated_staked_amount (actively earning rewards)
    ├── delegated_rewards_amount (accumulated rewards - also actively earning)
    ├── delegated_redelegated_amount (transitioning validators - also actively earning)
    └── delegated_unbonding_amount (21-day waiting period - not earning)
```
